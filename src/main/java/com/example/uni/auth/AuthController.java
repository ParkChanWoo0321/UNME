...
public class AuthController {
        ...
        try {
            ...
            try {
                ...
                if (u.isAbsolute()) {
                    ...
                    boolean allowed = host != null && (host.equals("likelionhsu.co.kr") || host.endsWith(".likelionhsu.co.kr") || host.equals("localhost"));
                    if (allowed) {
                        target = UriComponentsBuilder.fromUri(u).fragment("access=" + tokens.access()).build(true).toUri();
                    } else {
                        target = UriComponentsBuilder.fromUriString(frontendBase).fragment("access=" + tokens.access()).build(true).toUri();
                    }
                } else {
                    target = UriComponentsBuilder.fromUriString(frontendBase).path(s.startsWith("/") ? s : "/" + s).fragment("access=" + tokens.access()).build(true).toUri();
                }
            ...
            try {
                ...
                if (u.isAbsolute()) {
                    ...
                    boolean allowed = host != null && (host.equals("likelionhsu.co.kr") || host.endsWith(".likelionhsu.co.kr") || host.equals("localhost"));
                    if (allowed) {
                        target = UriComponentsBuilder.fromUri(u).fragment("error=ACCOUNT_DEACTIVATED").build(true).toUri();
                    } else {
                        target = UriComponentsBuilder.fromUriString(frontendBase).fragment("error=ACCOUNT_DEACTIVATED").build(true).toUri();
                    }
                } else {
                    target = UriComponentsBuilder.fromUriString(frontendBase).path(s.startsWith("/") ? s : "/" + s).fragment("error=ACCOUNT_DEACTIVATED").build(true).toUri();
                }
    ...
    public ResponseEntity<Map<String,Object>> me(@AuthenticationPrincipal String userId) {
        ...
        return ResponseEntity.ok(Map.of("firebaseCustomToken", firebaseCustomToken, "user", userService.toResponse(u)));
    }
