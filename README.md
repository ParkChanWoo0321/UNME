...
본 프로젝트는 개발 단계에서 끝난 것이 아니라, 실제 한서대학교 축제 기간 동안 배포되어 오프라인 부스 운영과 함께 사용된 서비스이다. 2025년 9월 29일부터 9월 30일까지 2일간 진행된 한서대학교 축제에서 동아리 부스를 운영하며 프로젝트를 실제 사용자에게 제공하였다.

축제 부스에서는 아이스티, 아메리카노 등의 음료를 판매하였고, 음료 1개를 구매한 사용자에게 이벤트 쿠폰 입력 칸에 사용할 수 있는 쿠폰 번호를 제공하였다. 사용자는 서비스에 접속하여 쿠폰 번호를 입력하고 이벤트에 참여할 수 있었으며, 이를 통해 오프라인 부스 운영과 온라인 서비스를 연결한 실제 사용자 참여형 서비스를 운영하였다.

운영 기간 동안 서비스 이용자가 빠르게 증가하여 총 1,200명 이상의 유저를 돌파하였다. 원래는 축제 종료일인 2025년 9월 30일까지만 운영할 예정이었지만, 사용자들의 추가 이용 요청이 이어져 운영 기간을 2025년 10월 3일까지 연장하였다. 이를 통해 단기간 이벤트성 서비스였음에도 실제 사용자 수요가 존재한다는 점을 확인할 수 있었다.
...
백엔드 개발 관점에서는 실제 사용자가 접속하는 환경을 고려하여 API 요청 처리, 쿠폰 입력 데이터 관리, 사용자 참여 기록 저장, 운영 중 데이터 확인이 가능하도록 서버 구조를 설계하였다. 특히 짧은 기간 동안 많은 사용자가 동시에 서비스를 이용할 수 있는 상황을 경험하면서, 단순한 기능 구현뿐만 아니라 실제 운영 환경에서 안정적으로 동작하는 백엔드 구조의 중요성을 확인하였다.
이번 프로젝트를 통해 백엔드 개발은 단순히 API를 만드는 것이 아니라, 실제 사용자의 행동 흐름과 서비스 운영 상황을 안정적으로 뒷받침하는 역할이라는 점을 경험하였다. 오프라인 행사와 온라인 서비스를 연결하고, 사용자 수 증가와 운영 기간 연장을 직접 겪으며 실사용 기반 프로젝트 운영 경험을 쌓을 수 있었다.
