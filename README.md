...
사용자는 `/auth/kakao/login`을 통해 카카오 인증 페이지로 이동하고, 콜백에서 카카오 사용자 정보를 조회합니다. 신규 사용자는 `users` 테이블에 저장되며, 기존 탈퇴 사용자는 재로그인을 차단합니다. 로그인 성공 시 Access Token은 프론트엔드 리다이렉트 URL의 fragment로 전달되고, Refresh Token은 HttpOnly Cookie로 저장됩니다.

### 4.2 프로필 등록 및 성향 분석

사진 넣을 곳

사용자는 이름, 학과, 학번, 출생 연도, 성별, MBTI, 10문항 성향 답변을 등록합니다. 서버는 답변을 기준으로 `typeId`와 `EGEN/TETO` 성향을 산출하고, OpenAI API를 통해 성향 요약, 추천 파트너 문장, 태그 3개를 생성합니다. OpenAI 호출 실패 시에도 기본 요약값을 반환하도록 fallback 로직을 두었습니다.

### 4.3 스마트 매칭

사진 넣을 곳

매칭은 사용자의 성별과 반대 성별을 대상으로 수행하며, 같은 학과 사용자는 제외합니다. 이미 본 후보, 이미 시그널을 보낸 사용자, 이미 채팅방이 존재하는 사용자는 제외하고 후보를 무작위로 섞어 최대 3명을 제공합니다. 후보가 존재할 때만 매칭 크레딧을 차감하고, 최근 매칭 결과를 `lastMatchJson`으로 저장합니다.

### 4.4 시그널 보내기 및 수락/거절

사진 넣을 곳

시그널은 관심 표현 기능입니다. 사용자가 특정 상대에게 시그널을 보내면 `signals` 테이블에 상태가 저장되고, 수신자에게 WebSocket 알림이 전송됩니다. 수신자는 시그널을 수락하거나 거절할 수 있으며, 수락 시 `MUTUAL` 상태 처리 후 Firestore 채팅방이 생성됩니다.

### 4.5 1:1 채팅방 자동 생성

사진 넣을 곳

시그널이 수락되면 `ChatRoomService`가 Firestore의 `chatRooms` 컬렉션에 두 사용자 기준의 고정 roomId를 생성합니다. roomId는 두 사용자 ID를 정렬한 `r_{id}_{id}` 형식으로 만들어 중복 채팅방 생성을 방지합니다. 채팅방에는 참여자, 상대 정보, 채팅 목록용 `listCard` 데이터가 함께 저장됩니다.

### 4.6 실시간 알림

사진 넣을 곳

STOMP WebSocket을 사용해 개인별 알림 큐를 제공합니다. 시그널 수신, 시그널 거절, 매칭 성사 이벤트가 발생하면 `/user/queue/signals`, `/user/queue/matches`로 알림을 전송합니다. WebSocket 연결 시에도 JWT를 검증해 탈퇴 사용자나 유효하지 않은 토큰의 연결을 차단합니다.

### 4.7 파일 업로드 및 유형 이미지 관리

사진 넣을 곳

프로필 이미지는 multipart 요청으로 업로드되며, 이미지 MIME 타입과 10MB 이하 크기를 검증합니다. 업로드된 파일은 `app.upload.dir` 아래 사용자별 디렉터리에 저장되고, 정적 리소스 핸들러를 통해 `/files/**` 경로로 제공됩니다. 유형 이미지는 `/admin/type-images/{type}` API를 통해 별도로 업로드할 수 있습니다.

### 4.8 랭킹 및 통계

사진 넣을 곳

시그널 전송 시 `signal_logs`, 매칭 성사 시 `match_logs`에 기록을 남깁니다. 이를 기반으로 학과별 시그널 랭킹, 학과별 매칭 랭킹, MBTI별 시그널 랭킹, MBTI별 매칭 랭킹을 조회할 수 있습니다.

### 4.9 이벤트 코드 사용

사진 넣을 곳

이벤트 코드는 `verify_code` 테이블에 저장된 1회성 코드를 사용합니다. 사용자가 코드를 입력하면 사용 여부를 원자적으로 갱신하고, 성공 시 매칭 크레딧과 시그널 크레딧을 각각 5개씩 증가시킵니다.

### 4.10 탈퇴 사용자 마스킹

사진 넣을 곳

사용자가 카카오 연결 해제 또는 탈퇴 처리를 하면 `deactivatedAt`을 기록하고 민감한 프로필 정보를 비웁니다. 기존 채팅방에는 탈퇴한 사용자의 이름과 이미지를 대체 값으로 반영해, 남아 있는 사용자에게 불필요한 개인정보가 노출되지 않도록 처리했습니다.
